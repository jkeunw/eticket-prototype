...
contract SmartTicket is ERC721{
    struct Ticket {
        uint256 eventId;
        uint256 seatId;
        bool isPaid;
    ...
    Ticket[] public tickets; // First ticket has index 0;
    ...
    function mintTicket(uint256 eventId, uint256 seatId, address account) public {
        require(owner == msg.sender); // Only the owner can create tickets
        uint256 ticketId = tickets.length; //Unique ticket id
        tickets.push(Ticket(eventId, seatId, true));
        _mint(account, ticketId); // Mint a new ticket
    }
}
